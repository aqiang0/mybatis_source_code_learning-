...
```
>看见熟悉的configuration，我们配置文件属性都在configuration标签之内，这里开始解析配置文件。   
> 
...
```
>这部分内容对configuration标签内的各标签顺序解析，每个标签基本Configuration对象都有对应的属性，这里解析完标签内容
...
```
>进来先判断是否已经解析过该xml文件，没有则开始解析mapper标签，对于mapper.xml，我们的所有配置都放在mapper标签之内。  
> 
...
```
>这个方法代码很长，但是不复杂。这里主要对增删改查语句解析，获取标签的各种属性，再封装成mappedStatement。标签属性可以参照[mybatis xml映射器](https://mybatis.net.cn/sqlmap-xml.html#select)，
